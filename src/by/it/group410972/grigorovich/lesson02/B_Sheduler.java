...
public class B_Sheduler {
    public static void main(String[] args) {
        ...
        System.out.println(starts);                                  // показываем расписание
    }
    ...
    List<Event> calcStartTimes(Event[] events, int from, int to) {
        ...
        for (Event event : events) {
            if (event.start >= currentTime && event.stop <= to) {
                result.add(event);
                currentTime = event.stop; // следующий интервал начинается не раньше окончания текущего
            }
        }
    ...
    // Событие (два поля: начало и конец)
    static class Event {
        int start;
        int stop;
